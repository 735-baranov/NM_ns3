...
public:
  ...
  void CreateInternet (NodeContainer);
private:
...
void
LoRaWANExampleTracing::CreateInternet (NodeContainer wifiApNode)
{
  Ptr<Node> APWiFi = wifiApNode.Get (0);
  Ptr<Node> GWLoRa = m_gatewayNodes.Get (1);
  ...
  NetDeviceContainer internetDevices = p2ph.Install (APWiFi, GWLoRa);
  ...
  // ApplicationContainer serverApps = echoServer.Install (internetDevices.Get (1));
  ...
  // ApplicationContainer clientApps = echoClient.Install (internetDevices.Get (0));
  // clientApps.Start (Seconds (2.0));
  // clientApps.Stop (Seconds (10.0));

  std::cout << "Simulation WIFI-LoRA done!" << std::endl;
}
...
//   cmd.AddValue ("verbose", "turn on all log components[Default:0]", verbose);
  ...
  for (uint32_t i = 0; i < nRuns; i++) {
    ...
    simSettingsFileName << simRunFilesPrefix.str() << "-sim-settings1.txt";
  ...
  if (standard == "11a")
    ...
    LoRaWANExampleTracing example1 = LoRaWANExampleTracing();
    example1.CreateInternet(wifiApNode);  
    ...
    std::string phyTransmissionTraceCSVFileName, std::string phyStateTraceCSVFileName, std::string macPacketTraceCSVFileName, std::string macStateTraceCSVFileName, std::string edMsgTraceCSVFileName, std::string nsDSMsgTraceCSVFileName, std::string miscTraceCSVFileName, std::string nodesCSVFileName)
{
  ...
  // netanim:
  AnimationInterface anim ("lorawan-netamin1.xml");
...
void
LoRaWANExampleTracing::CreateWiFi()
{
