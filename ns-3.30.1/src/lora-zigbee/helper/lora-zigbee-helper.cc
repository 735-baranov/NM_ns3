#include "lora-zigbee-helper.h"
#include <ns3/lora-zigbee-csmaca.h>
 ...
 */
static void
AsciiLoraZigBeeMacTransmitSinkWithContext (
 ...
 */
static void
AsciiLoraZigBeeMacTransmitSinkWithoutContext (
  Ptr<OutputStreamWrapper> stream,
  Ptr<const Packet> p)
{
...
LoraZigBeeHelper::LoraZigBeeHelper (bool useMultiModelSpectrumChannel)
{
...
void
LoraZigBeeHelper::EnableLogComponents (void)
{
  LogComponentEnableAll (LOG_PREFIX_TIME);
  LogComponentEnableAll (LOG_PREFIX_FUNC);
  LogComponentEnable ("LoraZigBeeCsmaCa", LOG_LEVEL_ALL);
  LogComponentEnable ("LoraZigBeeErrorModel", LOG_LEVEL_ALL);
  LogComponentEnable ("LoraZigBeeInterferenceHelper", LOG_LEVEL_ALL);
  LogComponentEnable ("LoraZigBeeMac", LOG_LEVEL_ALL);
  LogComponentEnable ("LoraZigBeeNetDevice", LOG_LEVEL_ALL);
  LogComponentEnable ("LoraZigBeePhy", LOG_LEVEL_ALL);
  LogComponentEnable ("LoraZigBeeSpectrumSignalParameters", LOG_LEVEL_ALL);
  LogComponentEnable ("LoraZigBeeSpectrumValueHelper", LOG_LEVEL_ALL);
}

std::string
LoraZigBeeHelper::LoraZigBeePhyEnumerationPrinter (LoraZigBeePhyEnumeration e)
{
  switch (e)
    {
...
std::string
LoraZigBeeHelper::LoraZigBeeMacStatePrinter (LoraZigBeeMacState e)
{
  switch (e)
    {
    case MAC_IDLE:
  ...
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); i++)
    {
      ...
      Ptr<LoraZigBeeNetDevice> netDevice = CreateObject<LoraZigBeeNetDevice> ();
      netDevice->SetChannel (m_channel);
      node->AddDevice (netDevice);
      netDevice->SetNode (node);
      // \todo add the capability to change short address, extended
      // address and panId. Right now they are hardcoded in LoraZigBeeMac::LoraZigBeeMac ()
...
void
LoraZigBeeHelper::AssociateToPan (NetDeviceContainer c, uint16_t panId)
{
  NetDeviceContainer devices;
  uint16_t id = 1;
  uint8_t idBuf[2];
  ...
  for (NetDeviceContainer::Iterator i = c.Begin (); i != c.End (); i++)
    {
      Ptr<LoraZigBeeNetDevice> device = DynamicCast<LoraZigBeeNetDevice> (*i);
      if (device)
        {
          idBuf[0] = (id >> 8) & 0xff;
          idBuf[1] = (id >> 0) & 0xff;
 ...
 */
static void
PcapSniffLoraZigBee (Ptr<PcapFileWrapper> file, Ptr<const Packet> packet)
{
  file->Write (Simulator::Now (), packet);
}

void
LoraZigBeeHelper::EnablePcapInternal (std::string prefix, Ptr<NetDevice> nd, bool promiscuous, bool explicitFilename)
{
  ...
  // NetDevice type
  //
  Ptr<LoraZigBeeNetDevice> device = nd->GetObject<LoraZigBeeNetDevice> ();
  if (device == 0)
    {
      NS_LOG_INFO ("LoraZigBeeHelper::EnablePcapInternal(): Device " << device << " not of type ns3::LoraZigBeeNetDevice");
      return;
    }
  ...
  if (promiscuous == true)
    {
      device->GetMac ()->TraceConnectWithoutContext ("PromiscSniffer", MakeBoundCallback (&PcapSniffLoraZigBee, file));

    }
  else
    {
      device->GetMac ()->TraceConnectWithoutContext ("Sniffer", MakeBoundCallback (&PcapSniffLoraZigBee, file));
    }
}

void
LoraZigBeeHelper::EnableAsciiInternal (
  Ptr<OutputStreamWrapper> stream,
  std::string prefix,
  Ptr<NetDevice> nd,
  bool explicitFilename)
{
  ...
  if (device == 0)
    {
      NS_LOG_INFO ("LoraZigBeeHelper::EnableAsciiInternal(): Device " << device << " not of type ns3::LoraZigBeeNetDevice");
      return;
    }
  ...
  if (stream == 0)
    {
      ...
      asciiTraceHelper.HookDefaultReceiveSinkWithoutContext<LoraZigBeeMac> (device->GetMac (), "MacRx", theStream);

      device->GetMac ()->TraceConnectWithoutContext ("MacTx", MakeBoundCallback (&AsciiLoraZigBeeMacTransmitSinkWithoutContext, theStream));

      asciiTraceHelper.HookDefaultEnqueueSinkWithoutContext<LoraZigBeeMac> (device->GetMac (), "MacTxEnqueue", theStream);
      asciiTraceHelper.HookDefaultDequeueSinkWithoutContext<LoraZigBeeMac> (device->GetMac (), "MacTxDequeue", theStream);
      asciiTraceHelper.HookDefaultDropSinkWithoutContext<LoraZigBeeMac> (device->GetMac (), "MacTxDrop", theStream);
  ...
  oss.str ("");
  oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid << "/$ns3::LoraZigBeeNetDevice/Mac/MacRx";
  device->GetMac ()->TraceConnect ("MacRx", oss.str (), MakeBoundCallback (&AsciiTraceHelper::DefaultReceiveSinkWithContext, stream));

  oss.str ("");
  oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid << "/$ns3::LoraZigBeeNetDevice/Mac/MacTx";
  device->GetMac ()->TraceConnect ("MacTx", oss.str (), MakeBoundCallback (&AsciiLoraZigBeeMacTransmitSinkWithContext, stream));

  oss.str ("");
  oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid << "/$ns3::LoraZigBeeNetDevice/Mac/MacTxEnqueue";
  device->GetMac ()->TraceConnect ("MacTxEnqueue", oss.str (), MakeBoundCallback (&AsciiTraceHelper::DefaultEnqueueSinkWithContext, stream));

  oss.str ("");
  oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid << "/$ns3::LoraZigBeeNetDevice/Mac/MacTxDequeue";
  device->GetMac ()->TraceConnect ("MacTxDequeue", oss.str (), MakeBoundCallback (&AsciiTraceHelper::DefaultDequeueSinkWithContext, stream));

  oss.str ("");
  oss << "/NodeList/" << nodeid << "/DeviceList/" << deviceid << "/$ns3::LoraZigBeeNetDevice/Mac/MacTxDrop";
