...
NS_LOG_COMPONENT_DEFINE ("LoraZigBeePhy");

NS_OBJECT_ENSURE_REGISTERED (LoraZigBeePhy);

// Table 22 in section 6.4.1 of ieee802.15.4
const uint32_t LoraZigBeePhy::aMaxPhyPacketSize = 127; // max PSDU in octets
const uint32_t LoraZigBeePhy::aTurnaroundTime = 12;  // RX-to-TX or TX-to-RX in symbol periods

// IEEE802.15.4-2006 Table 2 in section 6.1.2 (kb/s and ksymbol/s)
// The index follows LoraZigBeePhyOption
const LoraZigBeePhyDataAndSymbolRates
LoraZigBeePhy::dataSymbolRates[7] = { { 20.0, 20.0},
                                  { 40.0, 40.0},
                                  { 250.0, 12.5},
                                  { 250.0, 50.0},
                                  { 100.0, 25.0},
...
// The index follows LoraZigBeePhyOption
const LoraZigBeePhyPpduHeaderSymbolNumber
LoraZigBeePhy::ppduHeaderSymbolNumbers[7] = { { 32.0, 8.0, 8.0},
                                          { 32.0, 8.0, 8.0},
                                          { 2.0, 1.0, 0.4},
                                          { 6.0, 1.0, 1.6},
                                          { 8.0, 2.0, 2.0},
...
TypeId
LoraZigBeePhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LoraZigBeePhy")
    .SetParent<SpectrumPhy> ()
    .SetGroupName ("LoraZigBee")
    .AddConstructor<LoraZigBeePhy> ()
    .AddTraceSource ("TrxStateValue",
                     "The state of the transceiver",
                     MakeTraceSourceAccessor (&LoraZigBeePhy::m_trxState),
                     "ns3::TracedValueCallback::LoraZigBeePhyEnumeration")
    .AddTraceSource ("TrxState",
                     "The state of the transceiver",
                     MakeTraceSourceAccessor (&LoraZigBeePhy::m_trxStateLogger),
                     "ns3::LoraZigBeePhy::StateTracedCallback")
    .AddTraceSource ("PhyTxBegin",
                     "Trace source indicating a packet has "
                     "begun transmitting over the channel medium",
                     MakeTraceSourceAccessor (&LoraZigBeePhy::m_phyTxBeginTrace),
                     "ns3::Packet::TracedCallback")
    .AddTraceSource ("PhyTxEnd",
                     "Trace source indicating a packet has been "
                     "completely transmitted over the channel.",
                     MakeTraceSourceAccessor (&LoraZigBeePhy::m_phyTxEndTrace),
                     "ns3::Packet::TracedCallback")
    .AddTraceSource ("PhyTxDrop",
                     "Trace source indicating a packet has been "
                     "dropped by the device during transmission",
                     MakeTraceSourceAccessor (&LoraZigBeePhy::m_phyTxDropTrace),
                     "ns3::Packet::TracedCallback")
    .AddTraceSource ("PhyRxBegin",
                     "Trace source indicating a packet has begun "
                     "being received from the channel medium by the device",
                     MakeTraceSourceAccessor (&LoraZigBeePhy::m_phyRxBeginTrace),
                     "ns3::Packet::TracedCallback")
    .AddTraceSource ("PhyRxEnd",
                     "Trace source indicating a packet has been "
                     "completely received from the channel medium "
                     "by the device",
                     MakeTraceSourceAccessor (&LoraZigBeePhy::m_phyRxEndTrace),
                     "ns3::Packet::SinrTracedCallback")
    .AddTraceSource ("PhyRxDrop",
                     "Trace source indicating a packet has been "
                     "dropped by the device during reception",
                     MakeTraceSourceAccessor (&LoraZigBeePhy::m_phyRxDropTrace),
                     "ns3::Packet::TracedCallback")
  ;
  return tid;
}

LoraZigBeePhy::LoraZigBeePhy (void)
  : m_edRequest (),
    m_setTRXState ()
{
  ...
  LoraZigBeeSpectrumValueHelper psdHelper;
  ...
  m_signal = Create<LoraZigBeeInterferenceHelper> (m_noise->GetSpectrumModel ());
  m_rxLastUpdate = Seconds (0);
  Ptr<Packet> none_packet = 0;
  Ptr<LoraZigBeeSpectrumSignalParameters> none_params = 0;
...
LoraZigBeePhy::DoDispose (void)
{
  ...
  m_pdDataConfirmCallback = MakeNullCallback< void, LoraZigBeePhyEnumeration > ();
  m_plmeCcaConfirmCallback = MakeNullCallback< void, LoraZigBeePhyEnumeration > ();
  m_plmeEdConfirmCallback = MakeNullCallback< void, LoraZigBeePhyEnumeration,uint8_t > ();
  m_plmeGetAttributeConfirmCallback = MakeNullCallback< void, LoraZigBeePhyEnumeration, LoraZigBeePibAttributeIdentifier, LoraZigBeePhyPibAttributes* > ();
  m_plmeSetTRXStateConfirmCallback = MakeNullCallback< void, LoraZigBeePhyEnumeration > ();
  m_plmeSetAttributeConfirmCallback = MakeNullCallback< void, LoraZigBeePhyEnumeration, LoraZigBeePibAttributeIdentifier > ();

  SpectrumPhy::DoDispose ();
}

Ptr<NetDevice>
LoraZigBeePhy::GetDevice (void) const
{
  NS_LOG_FUNCTION (this);
  return m_device;
}
...
Ptr<MobilityModel>
LoraZigBeePhy::GetMobility (void)
{
  NS_LOG_FUNCTION (this);
  return m_mobility;
}
...
void
LoraZigBeePhy::SetDevice (Ptr<NetDevice> d)
{
...
void
LoraZigBeePhy::SetMobility (Ptr<MobilityModel> m)
{
...
void
LoraZigBeePhy::SetChannel (Ptr<SpectrumChannel> c)
{
...
LoraZigBeePhy::GetRxSpectrumModel (void) const
{
  NS_LOG_FUNCTION (this);
  if (m_txPsd)
    {
...
Ptr<AntennaModel>
LoraZigBeePhy::GetRxAntenna (void)
{
  NS_LOG_FUNCTION (this);
  return m_antenna;
}
...
void
LoraZigBeePhy::SetAntenna (Ptr<AntennaModel> a)
{
...
void
LoraZigBeePhy::StartRx (Ptr<SpectrumSignalParameters> spectrumRxParams)
{
  ...
  if (!m_edRequest.IsExpired ())
    {
      ...
      m_edPower.averagePower += LoraZigBeeSpectrumValueHelper::TotalAvgPower (m_signal->GetSignalPsd (), m_phyPIBAttributes.phyCurrentChannel) * (now - m_edPower.lastUpdate).GetTimeStep () / m_edPower.measurementLength.GetTimeStep ();
      m_edPower.lastUpdate = now;
    }

  Ptr<LoraZigBeeSpectrumSignalParameters> lorazigbeeRxParams = DynamicCast<LoraZigBeeSpectrumSignalParameters> (spectrumRxParams);

  if (lorazigbeeRxParams == 0)
    {
      CheckInterference ();
      ...
      if (!m_ccaRequest.IsExpired ())
        {
          double power = LoraZigBeeSpectrumValueHelper::TotalAvgPower (m_signal->GetSignalPsd (), m_phyPIBAttributes.phyCurrentChannel);
      ...
      Simulator::Schedule (spectrumRxParams->duration, &LoraZigBeePhy::EndRx, this, spectrumRxParams);
      return;
    }

  Ptr<Packet> p = (lorazigbeeRxParams->packetBurst->GetPackets ()).front ();
  NS_ASSERT (p != 0);
  ...
  if (m_trxState == IEEE_802_15_4_PHY_RX_ON && !m_setTRXState.IsRunning ())
    {
      ...
      // SINR.
      NS_LOG_DEBUG (this << " receiving packet with power: " << 10 * log10(LoraZigBeeSpectrumValueHelper::TotalAvgPower (lorazigbeeRxParams->psd, m_phyPIBAttributes.phyCurrentChannel)) + 30 << "dBm");
      m_signal->AddSignal (lorazigbeeRxParams->psd);
      Ptr<SpectrumValue> interferenceAndNoise = m_signal->GetSignalPsd ();
      *interferenceAndNoise -= *lorazigbeeRxParams->psd;
      *interferenceAndNoise += *m_noise;
      double sinr = LoraZigBeeSpectrumValueHelper::TotalAvgPower (lorazigbeeRxParams->psd, m_phyPIBAttributes.phyCurrentChannel) / LoraZigBeeSpectrumValueHelper::TotalAvgPower (interferenceAndNoise, m_phyPIBAttributes.phyCurrentChannel);
      ...
      if (10 * log10 (sinr) > -5)
        {
          ...
          m_currentRxPacket = std::make_pair (lorazigbeeRxParams, false);
          m_phyRxBeginTrace (p);
...
void
LoraZigBeePhy::CheckInterference (void)
{
  // Calculate whether packet was lost.
  LoraZigBeeSpectrumValueHelper psdHelper;
  Ptr<LoraZigBeeSpectrumSignalParameters> currentRxParams = m_currentRxPacket.first;
      ...
      if (m_errorModel != 0)
        {
          ...
          *interferenceAndNoise += *m_noise;
          double sinr = LoraZigBeeSpectrumValueHelper::TotalAvgPower (currentRxParams->psd, m_phyPIBAttributes.phyCurrentChannel) / LoraZigBeeSpectrumValueHelper::TotalAvgPower (interferenceAndNoise, m_phyPIBAttributes.phyCurrentChannel);
          ...
          LoraZigBeeLqiTag tag (std::numeric_limits<uint8_t>::max ());
...
void
LoraZigBeePhy::EndRx (Ptr<SpectrumSignalParameters> par)
{
  NS_LOG_FUNCTION (this);

  Ptr<LoraZigBeeSpectrumSignalParameters> params = DynamicCast<LoraZigBeeSpectrumSignalParameters> (par);
  ...
  if (currentRxParams == params)
    {
      ...
      Ptr<LoraZigBeeSpectrumSignalParameters> none = 0;
...
void
LoraZigBeePhy::PdDataRequest (const uint32_t psduLength, Ptr<Packet> p)
{
      ...
      if (m_trxState == IEEE_802_15_4_PHY_TX_ON)
        {
          ...
          Ptr<LoraZigBeeSpectrumSignalParameters> txParams = Create<LoraZigBeeSpectrumSignalParameters> ();
          ...
          m_pdDataRequest = Simulator::Schedule (txParams->duration, &LoraZigBeePhy::EndTx, this);
...
void
LoraZigBeePhy::PlmeCcaRequest (void)
{
  NS_LOG_FUNCTION (this);
  ...
  if (m_trxState == IEEE_802_15_4_PHY_RX_ON || m_trxState == IEEE_802_15_4_PHY_BUSY_RX)
    {
      ...
      m_ccaRequest = Simulator::Schedule (ccaTime, &LoraZigBeePhy::EndCca, this);
    }
  else
    {
...
void
LoraZigBeePhy::PlmeEdRequest (void)
{
  NS_LOG_FUNCTION (this);
  if (m_trxState == IEEE_802_15_4_PHY_RX_ON || m_trxState == IEEE_802_15_4_PHY_BUSY_RX)
    {
      ...
      m_edRequest = Simulator::Schedule (m_edPower.measurementLength, &LoraZigBeePhy::EndEd, this);
    }
  else
    {
      LoraZigBeePhyEnumeration result = m_trxState;
...
void
LoraZigBeePhy::PlmeGetAttributeRequest (LoraZigBeePibAttributeIdentifier id)
{
  NS_LOG_FUNCTION (this << id);
  LoraZigBeePhyEnumeration status;
  ...
  if (!m_plmeGetAttributeConfirmCallback.IsNull ())
    {
      LoraZigBeePhyPibAttributes retValue;
      memcpy (&retValue, &m_phyPIBAttributes, sizeof(LoraZigBeePhyPibAttributes));
...
// Section 6.2.2.7.3
void
LoraZigBeePhy::PlmeSetTRXStateRequest (LoraZigBeePhyEnumeration state)
{
      ...
      if ((m_trxState == IEEE_802_15_4_PHY_BUSY_RX) || (m_trxState == IEEE_802_15_4_PHY_RX_ON))
        {
          ...
          m_setTRXState = Simulator::Schedule (setTime, &LoraZigBeePhy::EndSetTRXState, this);
          return;
        }
...
bool
LoraZigBeePhy::ChannelSupported (uint8_t channel)
{
...
void
LoraZigBeePhy::PlmeSetAttributeRequest (LoraZigBeePibAttributeIdentifier id,
                                    LoraZigBeePhyPibAttributes* attribute)
{
  NS_LOG_FUNCTION (this << id << attribute);
  NS_ASSERT (attribute);
  LoraZigBeePhyEnumeration status = IEEE_802_15_4_PHY_SUCCESS;
...
void
LoraZigBeePhy::SetPdDataIndicationCallback (PdDataIndicationCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
LoraZigBeePhy::SetPdDataConfirmCallback (PdDataConfirmCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
LoraZigBeePhy::SetPlmeCcaConfirmCallback (PlmeCcaConfirmCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
LoraZigBeePhy::SetPlmeEdConfirmCallback (PlmeEdConfirmCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
LoraZigBeePhy::SetPlmeGetAttributeConfirmCallback (PlmeGetAttributeConfirmCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
LoraZigBeePhy::SetPlmeSetTRXStateConfirmCallback (PlmeSetTRXStateConfirmCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
LoraZigBeePhy::SetPlmeSetAttributeConfirmCallback (PlmeSetAttributeConfirmCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
LoraZigBeePhy::ChangeTrxState (LoraZigBeePhyEnumeration newState)
{
...
bool
LoraZigBeePhy::PhyIsBusy (void) const
{
...
void
LoraZigBeePhy::CancelEd (LoraZigBeePhyEnumeration state)
{
  NS_LOG_FUNCTION (this);
...
void
LoraZigBeePhy::EndEd (void)
{
  NS_LOG_FUNCTION (this);

  m_edPower.averagePower += LoraZigBeeSpectrumValueHelper::TotalAvgPower (m_signal->GetSignalPsd (), m_phyPIBAttributes.phyCurrentChannel) * (Simulator::Now () - m_edPower.lastUpdate).GetTimeStep () / m_edPower.measurementLength.GetTimeStep ();
...
void
LoraZigBeePhy::EndCca (void)
{
  NS_LOG_FUNCTION (this);
  LoraZigBeePhyEnumeration sensedChannelState = IEEE_802_15_4_PHY_UNSPECIFIED;
...
void
LoraZigBeePhy::EndSetTRXState (void)
{
  NS_LOG_FUNCTION (this);
...
Time
LoraZigBeePhy::CalculateTxTime (Ptr<const Packet> packet)
{
...
double
LoraZigBeePhy::GetDataOrSymbolRate (bool isData)
{
...
Time
LoraZigBeePhy::GetPpduHeaderTxTime (void)
{
  NS_LOG_FUNCTION (this);
...
void
LoraZigBeePhy::SetMyPhyOption (void)
{
  NS_LOG_FUNCTION (this);
...
LoraZigBeePhyOption
LoraZigBeePhy::GetMyPhyOption (void)
{
  NS_LOG_FUNCTION (this);
  return m_phyOption;
}
...
void
LoraZigBeePhy::SetTxPowerSpectralDensity (Ptr<SpectrumValue> txPsd)
{
...
void
LoraZigBeePhy::SetNoisePowerSpectralDensity (Ptr<const SpectrumValue> noisePsd)
{
...
LoraZigBeePhy::GetNoisePowerSpectralDensity (void)
{
  NS_LOG_FUNCTION (this);
  return m_noise;
}
...
void
LoraZigBeePhy::SetErrorModel (Ptr<LoraZigBeeErrorModel> e)
{
...
Ptr<LoraZigBeeErrorModel>
LoraZigBeePhy::GetErrorModel (void) const
{
  NS_LOG_FUNCTION (this);
  return m_errorModel;
}
...
uint64_t
LoraZigBeePhy::GetPhySHRDuration (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT (m_phyOption < IEEE_802_15_4_INVALID_PHY_OPTION);
...
double
LoraZigBeePhy::GetPhySymbolsPerOctet (void) const
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT (m_phyOption < IEEE_802_15_4_INVALID_PHY_OPTION);
...
int64_t
LoraZigBeePhy::AssignStreams (int64_t stream)
{
  NS_LOG_FUNCTION (this);
