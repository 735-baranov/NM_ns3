...
/**
 * \defgroup lora-zigbee LORA-ZIGBEE models
 *
...
struct McpsDataRequestParams
{
  ...
  LoraZigBeeAddressMode m_srcAddrMode; //!< Source address mode
  LoraZigBeeAddressMode m_dstAddrMode; //!< Destination address mode
 ...
 * \ingroup lora-zigbee
 *
 * Class that implements the LORA-ZIGBEE Mac state machine
 */
...
public:
   ...
   */
  void PlmeEdConfirm (LoraZigBeePhyEnumeration status, uint8_t energyLevel);
   ...
   */
  void PlmeGetAttributeConfirm (LoraZigBeePhyEnumeration status,
   ...
   */
  void PlmeSetTRXStateConfirm (LoraZigBeePhyEnumeration status);
   ...
   *  @param id the attributed identifier
   */
  void PlmeSetAttributeConfirm (LoraZigBeePhyEnumeration status,
                                LoraZigBeePibAttributeIdentifier id);
   ...
   */
  LoraZigBeeAssociationStatus GetAssociationStatus (void) const;
   ...
   */
  void SetAssociationStatus (LoraZigBeeAssociationStatus status);
  ...
  /**
   * TracedCallback signature for LoraZigBeeMacState change events.
   *
   * \param [in] oldValue The original state value.
   * \param [in] newValue The new state value.
   * \deprecated The LoraZigBeeMacState is now accessible as the
   * TracedValue \c MacStateValue. The \c MacState TracedCallback will
   * be removed in a future release.
   */
  typedef void (* StateTracedCallback)
    (LoraZigBeeMacState oldState, LoraZigBeeMacState newState);
...
private:
  ...
  /**
   * A trace source that fires when the LoraZigBeeMac changes states.
   ...
   * TracedValue.
   */
  TracedCallback<LoraZigBeeMacState, LoraZigBeeMacState> m_macStateLogger;
   ...
   */
  LoraZigBeeAssociationStatus m_associationStatus;
