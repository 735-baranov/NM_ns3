#ifndef LORA_ZIGBEE_PHY_H
#define LORA_ZIGBEE_PHY_H

#include "lora-zigbee-interference-helper.h"
...
class Packet;
class SpectrumValue;
class LoraZigBeeErrorModel;
struct LoraZigBeeSpectrumSignalParameters;
class MobilityModel;
...
} LoraZigBeePhyDataAndSymbolRates;
...
} LoraZigBeePhyPpduHeaderSymbolNumber;
...
} LoraZigBeePhyEnumeration;

namespace TracedValueCallback
{
/**
 * \ingroup lora-zigbee
 * TracedValue callback signature for LoraZigBeePhyEnumeration.
 *
 * \param [in] oldValue original value of the traced variable
 * \param [in] newValue new value of the traced variable
 */
  typedef void (* LoraZigBeePhyEnumeration)(LoraZigBeePhyEnumeration oldValue,
                                        LoraZigBeePhyEnumeration newValue);
}  // namespace TracedValueCallback
...
} LoraZigBeePibAttributeIdentifier;
 ...
 */
typedef Callback< void, LoraZigBeePhyEnumeration > PdDataConfirmCallback;
 ...
 */
typedef Callback< void, LoraZigBeePhyEnumeration > PlmeCcaConfirmCallback;
 ...
 */
typedef Callback< void, LoraZigBeePhyEnumeration,uint8_t > PlmeEdConfirmCallback;
 ...
 */
typedef Callback< void, LoraZigBeePhyEnumeration,
                  LoraZigBeePibAttributeIdentifier,
                  LoraZigBeePhyPibAttributes* > PlmeGetAttributeConfirmCallback;
 ...
 */
typedef Callback< void, LoraZigBeePhyEnumeration > PlmeSetTRXStateConfirmCallback;
 ...
 */
typedef Callback< void, LoraZigBeePhyEnumeration,
                  LoraZigBeePibAttributeIdentifier > PlmeSetAttributeConfirmCallback;

/**
 * \ingroup lora-zigbee
 *
 * Make LoraZigBeePhy a SpectrumPhy so we can enable the eventual modeling of
 * device interference
 */
class LoraZigBeePhy : public SpectrumPhy
{
...
public:
   ...
   */
  void PlmeGetAttributeRequest (LoraZigBeePibAttributeIdentifier id);
   ...
   */
  void PlmeSetTRXStateRequest (LoraZigBeePhyEnumeration state);
   ...
   */
  void PlmeSetAttributeRequest (LoraZigBeePibAttributeIdentifier id, LoraZigBeePhyPibAttributes* attribute);
   ...
   *
   * @param e pointer to LoraZigBeeErrorModel to use
   */
  void SetErrorModel (Ptr<LoraZigBeeErrorModel> e);

  /**
   * get the error model in use
   *
   * @return pointer to LoraZigBeeErrorModel in use
   */
  Ptr<LoraZigBeeErrorModel> GetErrorModel (void) const;
   ...
   * \deprecated The LoraZigBeePhyEnumeration state is now accessible as the
   * TracedValue \c TrxStateValue.  The \c TrxState TracedCallback will
   * be removed in a future release.
   */
  typedef void (* StateTracedCallback)
    (Time time, LoraZigBeePhyEnumeration oldState, LoraZigBeePhyEnumeration newState);
...
protected:
   ...
   */
  static const LoraZigBeePhyDataAndSymbolRates dataSymbolRates[7];
  /**
   * The preamble, SFD, and PHR lengths in symbols for the different PHY options.
   * See Table 19 and Table 20 in section 6.3 IEEE 802.15.4-2006
   */
  static const LoraZigBeePhyPpduHeaderSymbolNumber ppduHeaderSymbolNumbers[7];
...
private:
   ...
   */
  void ChangeTrxState (LoraZigBeePhyEnumeration newState);
   ...
   */
  LoraZigBeePhyOption GetMyPhyOption (void);
   ...
   */
  void CancelEd (LoraZigBeePhyEnumeration state);
   ...
   * be removed in a future release.
   */
  TracedCallback<Time, LoraZigBeePhyEnumeration, LoraZigBeePhyEnumeration> m_trxStateLogger;
   ...
   */
  Ptr<LoraZigBeeErrorModel> m_errorModel;

  /**
   * The current PHY PIB attributes.
   */
  LoraZigBeePhyPibAttributes m_phyPIBAttributes;

  // State variables
  /**
   * The current transceiver state.
   */
  TracedValue<LoraZigBeePhyEnumeration> m_trxState;

  /**
   * The next pending state to applied after the current action of the PHY is
   * completed.
   */
  LoraZigBeePhyEnumeration m_trxStatePending;
   ...
   */
  LoraZigBeePhyOption m_phyOption;

  /**
   * Helper value for tracking the average power during ED.
   */
  LoraZigBeeEdPower m_edPower;
   ...
   * considered noise.
   */
  Ptr<LoraZigBeeInterferenceHelper> m_signal;
   ...
   * due to interference.
   */
  std::pair<Ptr<LoraZigBeeSpectrumSignalParameters>, bool>  m_currentRxPacket;
