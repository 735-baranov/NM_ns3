...
LrWpanPhy::DoDispose (void)
{
  ...
  m_signal = 0;
  m_errorModel = 0;
  m_pdDataIndicationCallbackL = MakeNullCallback< void, uint32_t, Ptr<Packet>, uint8_t > ();
      ...
      if (!m_currentRxPacket.second)
        {
          ...
          if (!m_pdDataIndicationCallbackL.IsNull ())
            {
              m_pdDataIndicationCallbackL (currentPacket->GetSize (), currentPacket, tag.Get ());
            }
        }
      else
        {
...
void
LrWpanPhy::SetPdDataIndicationCallbackL (PdDataIndicationCallbackL c)
{
  NS_LOG_FUNCTION (this);
  m_pdDataIndicationCallbackL = c;
}
