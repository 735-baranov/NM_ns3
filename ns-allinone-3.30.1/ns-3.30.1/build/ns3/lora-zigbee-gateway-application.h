 ...
 */
  ...
  typedef void (* LoRaZiGbeeDSMessageTransmittedTracedCallback) (uint32_t deviceAddr, uint8_t txRemaining, uint8_t msgType, Ptr<const Packet> packet);
 ...
 */
  ...
  typedef void (* LoRaZiGbeeDSMessageTracedCallback) (uint32_t deviceAddr, uint8_t txRemaining, uint8_t msgType, Ptr<const Packet> packet);
}  // namespace TracedValueCallback

class Address;
class RandomVariableStream;
class Socket;
class LoRaZiGbeeGatewayApplication;

typedef struct LoRaZiGbeeNSDSQueueElement {
  Ptr<Packet>     m_downstreamPacket;
  uint8_t         m_downstreamFramePort;
  LoRaZiGbeeMsgType  m_downstreamMsgType;
  uint8_t 	  m_downstreamTransmissionsRemaining;
  bool 		  m_isRetransmission;
} LoRaZiGbeeNSDSQueueElement;

typedef struct LoRaZiGbeeEndDeviceInfoNS {
  LoRaZiGbeeEndDeviceInfoNS () : m_deviceAddress(), m_rx1DROffset(0), m_lastDSGW(nullptr), m_lastGWs(),
  ...
  Ptr<LoRaZiGbeeGatewayApplication> m_lastDSGW;
  std::vector< Ptr<LoRaZiGbeeGatewayApplication> > m_lastGWs;
  ...
  std::deque<LoRaZiGbeeNSDSQueueElement* > m_downstreamQueue;

  EventId 	  m_downstreamTimer; // DS traffic generator timer
} LoRaZiGbeeEndDeviceInfoNS;

//class LoRaZiGbeeNetworkServer : public SimpleRefCount<LoRaZiGbeeNetworkServer>
class LoRaZiGbeeNetworkServer : public Object
{
public:
  LoRaZiGbeeNetworkServer ();
  ...
  LoRaZiGbeeEndDeviceInfoNS InitEndDeviceInfo (Ipv4Address);

  static void clearLoRaZiGbeeNetworkServerPointer () { LoRaZiGbeeNetworkServer::m_ptr = nullptr; }
  static bool haveLoRaZiGbeeNetworkServerObject () { return LoRaZiGbeeNetworkServer::m_ptr != NULL; }
  static Ptr<LoRaZiGbeeNetworkServer> getLoRaZiGbeeNetworkServerPointer ();

  void SetConfirmedDataDown (bool confirmedData);
  bool GetConfirmedDataDown (void) const;

  void HandleUSPacket (Ptr<LoRaZiGbeeGatewayApplication>, Address from, Ptr<Packet> packet);
  void RW1TimerExpired (uint32_t deviceAddr);
  void RW2TimerExpired (uint32_t deviceAddr);
  void SendDSPacket (uint32_t deviceAddr, Ptr<LoRaZiGbeeGatewayApplication> gatewayPtr, bool RW1, bool RW2);
...
private:
  static Ptr<LoRaZiGbeeNetworkServer> m_ptr;
  std::unordered_map <uint32_t, LoRaZiGbeeEndDeviceInfoNS> m_endDevices;
  uint16_t m_pktSize;
...
class LoRaZiGbeeGatewayApplication : public Application
{
public:
  /**
  ...
  LoRaZiGbeeGatewayApplication ();

  virtual ~LoRaZiGbeeGatewayApplication();
...
private:
  ...
  Ptr<LoRaZiGbeeNetworkServer> m_lorazigbeeNSPtr; //!< Pointer to LoRaZiGbeeNetworkServer singleton
